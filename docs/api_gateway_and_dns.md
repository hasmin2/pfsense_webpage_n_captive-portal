...
| `dnsservergw_auto` | `"top2x2"` → 박스가 상위 2개 WAN에 DNS 2개씩 자동 배정 |
...
3. 온라인이 1개라도 있으면 **온라인 집합 안에서만** top-2 선택 → 항해 중 down된 LANDLINE 등 자동 배제
4. `dns[0],dns[1] → 1순위 WAN` / `dns[2],dns[3] → 2순위 WAN`
5. 폴백: 온라인 1개 → 그 1개에 전부 / 0개 → 타입우선순위(offline) / `vpn`·`metered_other`·disabled 제외
