<?php
/*
 * terminal_history_data.php (#57) — Terminal Status(terminal.php) 변경 이력 조회 JSON 엔드포인트
 *
 * terminal.php 의 HISTORY 버튼 모달이 AJAX 로 호출. 인증은 guiconfig.inc(관리 세션) 경유 —
 * 미인증이면 로그인 페이지로 리다이렉트되어 JSON 파싱 실패 → 모달이 오류 메시지 표시(무해).
 *
 * 파라미터(POST):
 *   mode=days   + days=1|7|30 (기본; 최근 N일)
 *   mode=custom + from=YYYY-MM-DD + to=YYYY-MM-DD (UTC 날짜 — 저장 timestamp 가 박스 UTC)
 *
 * 응답: {ok, rows:[{id,timestamp,admin_id,client_ip,description}...], from, to}
...
if (!function_exists('cp_terminal_history_fetch') && file_exists('/etc/inc/cp_terminal_history.inc')) {
    require_once('/etc/inc/cp_terminal_history.inc');
}
if (!function_exists('cp_terminal_history_fetch')) {
...
$rows = cp_terminal_history_fetch($from, $to);
if ($rows !== false) {
    $resp['ok']   = true;
    $resp['rows'] = $rows;
    $resp['from'] = $from;
